...
class FeatureFactory:
    ...
    def get_feature_names() -> list:
        names = list(FeatureFactory.FEATURE_NAMES)
        if bool(getattr(get_features(), "USE_OMEGA_TIMESTAMP_MISSING", False)):
            names.append('timestamp_missing')
        return names
    ...
    def extract(market_data: dict) -> dict:
        ...
        # Use quote timestamp for day_of_week (not wall-clock) so historical
        # training matches live inference and backtests are deterministic.
        _ts = market_data.get('timestamp')
        _timestamp_missing = 0.0
        if isinstance(_ts, datetime.datetime):
            _dow = _ts.weekday()
        elif isinstance(_ts, str):
            try:
                _dow = pd.to_datetime(_ts).weekday()
            except Exception:
                _dow = datetime.datetime.now().weekday()
                _timestamp_missing = 1.0  # wall-clock fallback
        else:
            _dow = datetime.datetime.now().weekday()
            _timestamp_missing = 1.0  # wall-clock fallback
        f['day_of_week'] = _dow / 4.0
        f['timestamp_missing'] = _timestamp_missing  # explicit flag for ML
        ...
        f['vix']           = vix / 100.0
        # Dynamic VIX z-score: use rolling history when available (passes from
        # upstream market data), otherwise fall back to long-run India VIX stats.
        _vix_hist = market_data.get('vix_history', [])
        if len(_vix_hist) >= 20:
            vix_mean = float(np.mean(_vix_hist[-252:]))
            vix_std  = max(float(np.std(_vix_hist[-252:])), 1.0)
        else:
            vix_mean = 14.0   # long-run India VIX mean (2003–2024)
            vix_std  = 5.0    # historical India VIX std deviation
        f['vix_zscore']    = (vix - vix_mean) / vix_std
    ...
    def to_array(features: dict) -> np.ndarray:
        ...
        names = FeatureFactory.get_feature_names()
...
class MLPricingCorrector:
    ...
    def __init__(self, model_path='omega_data/pricing_model.joblib'):
        ...
        self._pre_drift_model = None  # rollback checkpoint
        # Urgency tracker: stores (timestamp, residual) pairs to estimate half-life
        self._residual_history = []  # list of (timestep_idx, abs_residual)
        self._schema_mismatch = False
        self._load_issue = None
        self._load_model()

    # ------------------------------------------------------------------
    def _current_feature_schema(self) -> dict:
        names = FeatureFactory.get_feature_names()
        return {
            'feature_names': names,
            'feature_count': len(names),
            'timestamp_missing_enabled': bool(
                getattr(get_features(), "USE_OMEGA_TIMESTAMP_MISSING", False)
            ),
        }
    ...
    def _load_model(self):
        ...
        try:
            ...
            saved_x = saved.get('X', [])
            saved_schema = saved.get('feature_schema', {})
            cur_schema = self._current_feature_schema()
            cur_names = list(cur_schema.get('feature_names', []))
            cur_count = int(cur_schema.get('feature_count', len(cur_names)))
            saved_names = list(saved_schema.get('feature_names', [])) if isinstance(saved_schema, dict) else []
            if isinstance(saved_schema, dict) and 'feature_count' in saved_schema:
                saved_count = int(saved_schema.get('feature_count', 0))
            else:
                try:
                    saved_count = len(saved_x[0]) if saved_x else cur_count
                except Exception:
                    saved_count = cur_count

            schema_ok = (saved_count == cur_count)
            if saved_names and saved_names != cur_names:
                schema_ok = False

            if not schema_ok:
                self._schema_mismatch = True
                self._load_issue = (
                    f"feature_schema_mismatch(saved={saved_count}, expected={cur_count})"
                )
                self.model = None
                self.is_trained = False
                self.training_X = []
            ...
            self.training_X = saved_x
            self.training_y = saved.get('y', [])
            self.training_w = saved.get('w', [])
            self._conformal_q_global = saved.get('conformal_q_global', None)
            self._conformal_q_by_regime = saved.get('conformal_q_by_regime', {})
            fm = saved.get('feature_means')
            fs = saved.get('feature_stds')
            self._feature_means = np.asarray(fm, dtype=float) if fm is not None else None
            self._feature_stds = np.asarray(fs, dtype=float) if fs is not None else None
            self._schema_mismatch = False
            self._load_issue = None
            self.is_trained = bool(self.model is not None)
        except Exception:
            self._schema_mismatch = True
            self._load_issue = 'model_load_failed'
            self.is_trained = False
    ...
    def _save_model(self):
        ...
        try:
            ...
            joblib.dump({
                ...
                'w': getattr(self, 'training_w', [])[-2000:],
                'conformal_q_global': self._conformal_q_global,
                'conformal_q_by_regime': self._conformal_q_by_regime,
                'feature_schema': self._current_feature_schema(),
                'feature_means': (
                    self._feature_means.tolist() if isinstance(self._feature_means, np.ndarray) else None
                ),
                'feature_stds': (
                    self._feature_stds.tolist() if isinstance(self._feature_stds, np.ndarray) else None
                ),
            }, self.model_path)
        except Exception:
            pass
    ...
    def predict_correction(self, features: dict):
        ...
        try:
            ...
            # Adaptive correction cap: tighter for ATM, looser for deep OTM
            _moneyness = abs(features.get('moneyness', 1.0) - 1.0)
            _cap = 0.10 + 0.10 * min(_moneyness * 10.0, 1.0)  # 10% ATM, 20% deep OTM
            correction = np.clip(correction, -_cap, _cap)
    ...
    def predict_urgency(self, features: dict) -> dict:
        """
        Estimate the half-life of a mispricing: how quickly will the gap close?

        Returns dict with:
          half_life_minutes : float — estimated time for mispricing to halve
          entry_strategy    : str — 'MARKET_TAKE' (fast close) or 'MARKET_MAKE' (slow close)
          urgency_score     : float — 0-100 (100 = enter immediately)
        """
        if len(self._residual_history) < 10:
            return {'half_life_minutes': 60.0, 'entry_strategy': 'MARKET_MAKE', 'urgency_score': 30.0}

        # Estimate decay rate from historical residuals using log-linear regression
        recent = self._residual_history[-50:]
        t_arr = np.array([r[0] for r in recent], dtype=float)
        r_arr = np.array([r[1] for r in recent], dtype=float)
        r_arr = np.maximum(r_arr, 1e-8)
        log_r = np.log(r_arr)

        try:
            # Linear regression: log(residual) = a + b*t  →  half_life = -ln(2)/b
            coeffs = np.polyfit(t_arr - t_arr[0], log_r, 1)
            decay_rate = -coeffs[0]  # positive means decaying
            if decay_rate > 0:
                half_life_steps = np.log(2) / decay_rate
                half_life_minutes = max(0.5, half_life_steps * 5.0)  # assume ~5 min per step
            else:
                half_life_minutes = 300.0  # not decaying, persist
        except Exception:
            half_life_minutes = 60.0

        if half_life_minutes < 10:
            strategy = 'MARKET_TAKE'   # cross the spread, mispricing closes fast
            urgency = min(100, 90 + (10 - half_life_minutes))
        elif half_life_minutes < 60:
            strategy = 'MARKET_MAKE'   # sit on the bid, mispricing closes slowly
            urgency = max(30, 70 - half_life_minutes)
        else:
            strategy = 'MARKET_MAKE'
            urgency = max(10, 30 - half_life_minutes / 10)

        return {
            'half_life_minutes': round(float(half_life_minutes), 1),
    ...
    def check_drift(self, features: dict) -> dict:
        """
        Feature drift monitor: compares current feature vector against
        the training distribution. Returns drift score and rollback flag.
        """
        ...
        if len(X) != len(self._feature_means):
            return {
                'drift_score': 1.0,
                'max_feature_z': 999.0,
                'drift_detected': True,
                'n_drifted_features': int(max(len(X), len(self._feature_means))),
                'reason': 'schema_mismatch',
            }
        # Z-score each feature against training distribution
        z_scores = np.abs((X - self._feature_means) / np.maximum(self._feature_stds, 1e-8))
        # Fraction of features with |z| > 3 (significant drift)
        drift_frac = float(np.mean(z_scores > 3.0))
        max_z = float(np.max(z_scores))

        drift_detected = drift_frac > 0.2 or max_z > 6.0
        self._drift_detected = drift_detected

        # Auto-rollback: if drift detected and we have a pre-drift checkpoint
        if drift_detected and self._pre_drift_model is not None:
            self.model = self._pre_drift_model
            warnings.warn("OMEGA drift detected: rolled back to pre-drift model checkpoint")

        return {
            'drift_score': round(drift_frac, 3),
            'max_feature_z': round(max_z, 2),
            'drift_detected': drift_detected,
            'n_drifted_features': int(np.sum(z_scores > 3.0)),
        }

    def predict_correction_with_interval(self, features: dict, alpha: float = 0.10):
        """
        Conformalized Quantile Regression (CQR) prediction interval.

        Returns (correction, confidence, lower, upper).

        Provides a statistical coverage guarantee: P(true ∈ [lower, upper]) ≥ 1-α
        regardless of the underlying distribution. Intervals are adaptive —
        wider for OTM options in volatile regimes, narrower for ATM in calm.
        ...
        # --- CQR: start with the global conformal quantile ---
        q = self._conformal_q_global if self._conformal_q_global is not None else 0.0

        # --- Regime-binned quantile (tighter within regime) ---
        regime = 'Sideways'
        ...
        # --- Regime × moneyness quantile (most specific) ---
        log_m_val = features.get('log_moneyness', 0.0)
        if log_m_val < -0.03:
            m_bucket = 'ITM'
        elif log_m_val > 0.03:
            m_bucket = 'OTM'
            ...
            m_bucket = 'ATM'
        q_rm = getattr(self, '_conformal_q_by_regime_moneyness', {}).get((regime, m_bucket))
        if q_rm is not None and np.isfinite(q_rm):
            q = float(q_rm)  # use the most specific quantile

        q = max(float(q), 0.0)

        # --- Adaptive width modulation ---
        # CQR insight: multiply base width by local difficulty score
        # Factors that increase uncertainty:
        #   1. Deep OTM/ITM (moneyness far from 1.0)
        #   2. Near-expiry (gamma risk amplifies errors)
        #   3. High-vol regime (Bear-High Vol)
        #   4. Low liquidity / wide spreads

        # Moneyness factor: OTM options are harder to price
        log_m = abs(features.get('log_moneyness', 0.0))
        moneyness_mult = 1.0 + 1.5 * min(log_m, 0.3)  # [1.0, 1.45]

        # Time factor: near-expiry options have larger model error
        T = max(features.get('time_to_expiry', 0.05), 1e-6)
        time_mult = 1.0 + 0.3 * max(0, 1.0 / (T * 52) - 1.0)  # boost for < 1 week
        time_mult = min(time_mult, 1.6)

        # Regime factor: volatile regimes → wider intervals
        regime_mult = 1.0
        if regime == 'Bear-High Vol':
            regime_mult = 1.3
        elif regime == 'Bull-High Vol':
            regime_mult = 1.1

        # GEX Gamma Flip integration: if spot < zero_gamma_strike,
        # dealers are short gamma → volatility amplified → widen intervals 2x
        gamma_flip_mult = 1.0
        _gex_flip = features.get('gex_gamma_flip_strike', None)
        _spot = features.get('spot_price', None)
        if _gex_flip is not None and _spot is not None and _spot < _gex_flip:
            gamma_flip_mult = 2.0  # double width when below gamma flip

        adaptive_q = q * moneyness_mult * time_mult * regime_mult * gamma_flip_mult

        # --- Coverage calibration with finite-sample correction ---
        # Vovk (2012): for n calibration points, use (1-α)(1 + 1/n) quantile
        n_cal = max(len(self.training_y), 1)
        coverage_correction = (1.0 + 1.0 / n_cal)
        adaptive_q *= min(coverage_correction, 1.05)  # cap correction at 5%

        return corr, conf, corr - adaptive_q, corr + adaptive_q

    # ------------------------------------------------------------------
    def add_sample(self, features: dict, residual: float, weight: float = 1.0):
        ...
        if not hasattr(self, 'training_w'):
            self.training_w = [1.0] * (len(self.training_y) - 1)
        self.training_w.append(float(np.clip(weight, 0.0, 1.0)))
    ...
    def _train(self):
        ...
        if self.model is not None:
            try:
                self._pre_drift_model = copy.deepcopy(self.model)
            except Exception:
                self._pre_drift_model = self.model
        X = np.array(self.training_X)
        y = np.array(self.training_y)
        w = np.array(self.training_w if hasattr(self, 'training_w') and len(getattr(self, 'training_w', [])) == len(y) else [1.0] * len(y))
        ...
        # Collect out-of-fold predictions for rigorous conformal calibration
        oof_preds = np.full_like(y, np.nan)

        for train_idx, val_idx in tscv.split(X_s):
            fold_model = _make_model(len(train_idx))
            fold_model.fit(X_s[train_idx], y[train_idx], sample_weight=w[train_idx])
            score = fold_model.score(X_s[val_idx], y[val_idx])
            cv_scores.append(max(score, 0.0))
            oof_preds[val_idx] = fold_model.predict(X_s[val_idx])
        ...
        self.model.fit(X_s, y, sample_weight=w)

        # Conformal residual quantiles — use OUT-OF-FOLD predictions so
        # intervals are not artificially tight from in-sample overfitting.
        try:
            oof_mask = np.isfinite(oof_preds)
            if np.any(oof_mask):
                abs_res = np.abs(y[oof_mask] - oof_preds[oof_mask])
            else:
                # Fallback to in-sample if no OOF available
                abs_res = np.abs(y - self.model.predict(X_s))
            self._conformal_q_global = float(np.quantile(abs_res, 0.90)) if len(abs_res) > 0 else 0.0

            self._conformal_q_by_regime = {}
            self._conformal_q_by_regime_moneyness = {}  # regime×moneyness grid
            ...
            lm_col = FeatureFactory.FEATURE_NAMES.index('log_moneyness')
            X_raw = np.array(self.training_X)
            if np.any(oof_mask):
                abs_res_full = np.full_like(y, np.nan)
                abs_res_full[oof_mask] = abs_res if len(abs_res) == np.sum(oof_mask) else np.abs(y[oof_mask] - oof_preds[oof_mask])
            else:
                abs_res_full = np.abs(y - self.model.predict(X_s))
            if X_raw.ndim == 2 and X_raw.shape[1] >= max(max(reg_cols), lm_col) + 1:
                reg_idx = np.argmax(X_raw[:, reg_cols], axis=1)
                log_m_vals = X_raw[:, lm_col]
                # Moneyness buckets: ITM (<-0.03), ATM (-0.03..0.03), OTM (>0.03)
                m_buckets = {'ITM': log_m_vals < -0.03, 'ATM': (log_m_vals >= -0.03) & (log_m_vals <= 0.03), 'OTM': log_m_vals > 0.03}
                for i, rn in enumerate(reg_names):
                    r_mask = (reg_idx == i) & np.isfinite(abs_res_full)
                    if np.sum(r_mask) >= 20:
                        self._conformal_q_by_regime[rn] = float(np.quantile(abs_res_full[r_mask], 0.90))
                    # Regime × moneyness
                    for mb_name, mb_mask in m_buckets.items():
                        joint = r_mask & mb_mask
                        if np.sum(joint) >= 10:
                            self._conformal_q_by_regime_moneyness[(rn, mb_name)] = float(np.quantile(abs_res_full[joint], 0.90))
        except Exception:
...
class EfficiencyHunter:
    ...
    def score(self, features: dict, ce_price=0.0, pe_price=0.0, market_spot=None) -> dict:
        # Derive spot from market_spot if available, else from strike/moneyness
        if market_spot is not None and float(market_spot) > 0:
            spot = float(market_spot)
        else:
            strike = features.get('strike', features.get('K', 0.0))
            moneyness = features.get('moneyness', 1.0)
            spot = float(strike / moneyness) if moneyness > 0 and strike > 0 else float(max(features.get('spot', 1.0), 1e-8))
        result = {
            'anomaly_score': 0.0,
            'iv_anomaly': 0.0,
        ...
        if ce_price > 0 and pe_price > 0:
            # moneyness = K/S in FeatureFactory, so K = S * moneyness
            S = float(market_spot) if (market_spot is not None and market_spot > 0) else 24000.0
            moneyness = features.get('moneyness', 1.0)
            K = S * moneyness
...
class OMEGAModel:
    ...
    def price_option(self, spot, strike, T, r, q, option_type, market_price,
        ...
        urgency_info = {'enabled': False, 'reason': 'ml_unavailable'}
        drift_info = {'enabled': False, 'reason': 'ml_unavailable'}
        if self.ml:
            if bool(getattr(self.ml, "_schema_mismatch", False)):
                _issue = str(getattr(self.ml, "_load_issue", "schema_mismatch")).upper()
                actionability_reasons.append(f"ML_FAILSAFE_{_issue}")
            if not bool(self.ml.is_trained):
                actionability_reasons.append("ML_CORRECTOR_UNTRAINED")

            if getattr(get_features(), "USE_OMEGA_URGENCY", False):
                urgency_info = self.ml.predict_urgency(features)
                urgency_info['enabled'] = True
            else:
                urgency_info = {'enabled': False, 'reason': 'flag_off'}

            drift_detected = False
            if getattr(get_features(), "USE_OMEGA_DRIFT_GUARD", False):
                drift_info = self.ml.check_drift(features)
                drift_info['enabled'] = True
                drift_detected = bool(drift_info.get('drift_detected', False))
                if drift_detected:
                    actionability_reasons.append("ML_DRIFT_DETECTED_CORRECTION_SUPPRESSED")
            else:
                drift_info = {'enabled': False, 'reason': 'flag_off'}

            use_ml = not bool(getattr(self.ml, "_schema_mismatch", False)) and not drift_detected
            if use_ml:
        ...
        efficiency = self.hunter.score(features, market_spot=spot)
        ...
        if getattr(get_features(), "USE_CONFORMAL_INTERVALS", False):
            ...
            if not liquidity_ok:
                actionability_reasons.append("LOW_LIQUIDITY_FOR_CONFORMAL_GATE")
            ...
            if not outside_interval:
                actionability_reasons.append("EDGE_INSIDE_CONFORMAL_INTERVAL")
        ...
        if (
            ...
            omega_signal = 'HOLD'
            actionability_reasons.append("FILTERED_BY_HIGH_CONVICTION_GATE")
        ...
        if getattr(get_features(), "USE_OOS_RELIABILITY_GATE", False):
            ...
            if oos_gate.get('required') and not oos_gate.get('passed'):
                actionability_reasons.append("BLOCKED_BY_OOS_RELIABILITY_GATE")

        if omega_signal != 'HOLD' and not actionability_reasons:
    ...
    # ==================================================================
    def _enhanced_signal(self, misp, phys_pop, conf, eff_score, sent, ml_conf, ml_corr=0.0):
        buy = sell = 0
        ...
        # ML confidence boost — require directional alignment so data quantity
        # alone cannot inflate a signal that the ML correction contradicts.
        if ml_conf > 0.5 and abs(ml_corr) > 0.01:
            if buy > sell and ml_corr > 0: buy += 1
            elif sell > buy and ml_corr < 0: sell += 1
    ...
    def learn_from_outcome(self, prediction_id, actual_return, actual_price=None):
        ...
        for p in reversed(self.tracker.predictions):
            if p['id'] == prediction_id and p.get('features'):
                if self.ml:
                    # The ML corrector learns PRICING RESIDUALS, not trade returns.
                    # residual = (actual_price - model_fair_value) / model_fair_value
                    # This aligns with what `predict_correction` outputs.
                    base_fv = p.get('base_fair_value', 0)
                    if actual_price is not None and base_fv > 0:
                        residual = (float(actual_price) - float(base_fv)) / float(base_fv)
                        self.ml.add_sample(p['features'], residual, weight=1.0)
                    else:
                        residual = float(actual_return)  # fallback
                        self.ml.add_sample(p['features'], residual, weight=0.3)
                break
    ...
    def scan_chain(self, spot, strikes, T, r, q,
        ...
        """
        bid_quotes_ce = kwargs.get("bid_quotes_ce") if isinstance(kwargs.get("bid_quotes_ce"), dict) else {}
        ask_quotes_ce = kwargs.get("ask_quotes_ce") if isinstance(kwargs.get("ask_quotes_ce"), dict) else {}
        voi_quotes_ce = kwargs.get("volume_oi_ratio_ce") if isinstance(kwargs.get("volume_oi_ratio_ce"), dict) else {}
        bid_quotes_pe = kwargs.get("bid_quotes_pe") if isinstance(kwargs.get("bid_quotes_pe"), dict) else {}
        ask_quotes_pe = kwargs.get("ask_quotes_pe") if isinstance(kwargs.get("ask_quotes_pe"), dict) else {}
        voi_quotes_pe = kwargs.get("volume_oi_ratio_pe") if isinstance(kwargs.get("volume_oi_ratio_pe"), dict) else {}
        ...
        for _k in (
            ...
            shared_kwargs.pop(_k, None)

        results = []
        for strike in strikes:
            if strike in market_prices_ce and market_prices_ce[strike] > 0:
                try:
                    ce_kwargs = dict(shared_kwargs)
                    if strike in bid_quotes_ce:
                        ce_kwargs["bid"] = bid_quotes_ce[strike]
                    if strike in ask_quotes_ce:
                        ce_kwargs["ask"] = ask_quotes_ce[strike]
                    if strike in voi_quotes_ce:
                        ce_kwargs["volume_oi_ratio"] = voi_quotes_ce[strike]
                    res = self.price_option(
                        spot, strike, T, r, q, 'CE', market_prices_ce[strike],
                        india_vix, fii_net_flow, dii_net_flow, days_to_rbi,
                        pcr_oi, returns_30d, **ce_kwargs)
            ...
            if strike in market_prices_pe and market_prices_pe[strike] > 0:
                try:
                    pe_kwargs = dict(shared_kwargs)
                    if strike in bid_quotes_pe:
                        pe_kwargs["bid"] = bid_quotes_pe[strike]
                    if strike in ask_quotes_pe:
                        pe_kwargs["ask"] = ask_quotes_pe[strike]
                    if strike in voi_quotes_pe:
                        pe_kwargs["volume_oi_ratio"] = voi_quotes_pe[strike]
                    res = self.price_option(
                        spot, strike, T, r, q, 'PE', market_prices_pe[strike],
                        india_vix, fii_net_flow, dii_net_flow, days_to_rbi,
                        pcr_oi, returns_30d, **pe_kwargs)
