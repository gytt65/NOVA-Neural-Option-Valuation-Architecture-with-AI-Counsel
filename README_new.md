...
## Runtime Safety Status (Hardening Pass)

This codebase is intended for **manual execution support**.  
Signals are advisory; trader action remains manual.

- `implemented + active (when enabled)`: OMEGA urgency/drift call-path wiring, NOVA BSE validation path, NOVA shadow feedback lifecycle, executable spread/liquidity gating, baseline-only mode pass-through.
- `implemented + flagged (default OFF)`: `USE_OMEGA_TIMESTAMP_MISSING`, `USE_OMEGA_URGENCY`, `USE_OMEGA_DRIFT_GUARD`, `USE_NOVA_BSE_VALIDATION`, `USE_NOVA_SHADOW_FEEDBACK`.
- `implemented + integrated`: Levy regularization in calibration objective, macro release-calendar alignment in enhanced pricing, contagion weak-link threshold filtering, copula tail-stability confidence degradation.
- `implemented + guarded`: AdaptiveMesh targeted-only enforcement blocks accidental full-chain usage without explicit override.
- `implemented + active`: AI Council reliability updates now use prior-cycle forecasts vs realized spot movement and persist Brier-calibrated reliability.
- `implemented + active`: AI Council quality telemetry writes `seat_reliability`, `provider_reliability`, and `evidence_quality` each cycle.
- `implemented + active`: AI Council cost-capped mode now tracks daily provider spend and shifts to low-cost providers near budget limits.
- `implemented + active`: AI Council stale-evidence guard now forces HOLD when directional citations are missing timestamped evidence metadata.
- `implemented + active`: AI Council provider-degraded logic now keys off configured providers/failure findings to avoid false strict-HOLD behavior.
