...
class DynamicSABR:
    ...
    def calibrate_slice(self, F, strikes, market_ivs, T, expiry_key=None):
        ...
        # Warm-start from prior slice if available (cross-maturity continuity)
        atm_idx = np.argmin(np.abs(strikes - F))
        alpha0 = float(market_ivs[atm_idx] * F**(1.0 - beta))
        rho0, nu0 = -0.3, 0.4
        if self.calibrated_params:
            prev = list(self.calibrated_params.values())[-1]
            rho0, nu0 = prev.get('rho', rho0), prev.get('nu', nu0)
            alpha0 = prev.get('alpha', alpha0)
        ...
        try:
            ...
            # Cross-maturity no-arb check: total variance must be non-decreasing
            if expiry_key and self.calibrated_params:
                for ek, ep in self.calibrated_params.items():
                    if 'total_var' in ep:
                        new_total_var = alpha**2 * T
                        if new_total_var < ep['total_var'] * 0.95:
                            warnings.warn(f"SABR calendar arb: total variance decreased {ek}->{expiry_key}")

            params = {'alpha': float(alpha), 'rho': float(rho),
                      'nu': float(nu), 'beta': beta, 'rmse': rmse,
                      'total_var': float(alpha**2 * T)}
...
class GJRGarch:
    ...
    def fit(self, returns, horizon=1):
        ...
        if ARCH_AVAILABLE:
            try:
                ...
                # Vol cap/floor guardrails
                ann_vol = float(np.clip(ann_vol, 0.03, 2.0))
    ...
    def _fallback_forecast(self, returns, horizon):
        ...
        ann_vol = float(np.clip(daily_vol * np.sqrt(252), 0.03, 2.0))
        self.forecast_vol = ann_vol
...
class HestonCOS:
    ...
    def _cumulant_c4_numerical(self, S, K, T, r, q, V0, kappa, theta, sigma_v, rho):
        ...
        try:
            ...
            # Guard: c4 must be non-negative (excess kurtosis); NaN/negative → Gaussian fallback
            if not np.isfinite(c4) or c4 < 0:
                c4 = 0.0
            return c4
        except Exception:
            return 0.0
    ...
    def calibrate(self, S, strikes, market_prices, T, r, q, option_types=None):
        ...
        def objective(params):
            ...
            # Hard enforcement: reject violators with severe penalty
            feller_margin = 2.0 * kappa * theta - sigma_v**2
            if feller_margin < 0:
                err += 50.0 * feller_margin**2  # strong penalty
            per_strike_resid = []
            for _i, (K_i, mkt, ot) in enumerate(zip(strikes, market_prices, option_types)):
                try:
                    ...
                    resid = ((model - mkt) / max(mkt, 0.1))**2
                    err += resid
                    per_strike_resid.append(resid)
                except Exception:
                    err += 100.0
                    per_strike_resid.append(100.0)
            # Store calibration residuals for diagnostics
            objective._last_residuals = per_strike_resid
            return err
        objective._last_residuals = []
        ...
        try:
            ...
            return {
                ...
                'per_strike_residuals': getattr(objective, '_last_residuals', []),
...
class EMJumpEstimator:
    ...
    def fit(self, returns, dt=1.0/252):
        ...
        n = len(returns)
        if n < 30:
            # Shrinkage prior: blend with uninformative prior for small samples
            base = self._default_params()
            if n >= 10:
                empirical_std = float(np.std(returns))
                shrinkage = n / 60.0  # gradual transition to pure MLE
                base['sigma_d'] = base['sigma_d'] * (1 - shrinkage) + empirical_std * shrinkage
            return base
...
class MLSignalPipeline:
    ...
    def train(self, X_list, y_list, purge_gap=5):
        ...
        embargo = max(2, purge_gap)  # embargo period to prevent autocorrelation leakage
        for train_idx, val_idx in tscv.split(X_scaled):
            ...
            # Embargo: also remove `embargo` samples from the start of validation
            if embargo > 0 and len(val_idx) > embargo + 5:
...
class ContinuousRegimeDetector:
    ...
    def __init__(self, vix_mid=16.0, k=0.3, hysteresis=1.5):
        """
        Parameters
        ----------
        vix_mid    : float - VIX level at 50% turbulent probability
        k          : float - Sigmoid steepness
        hysteresis : float - Band width to prevent flip noise
        """
        self.vix_mid = vix_mid
        self.k = k
        self.hysteresis = hysteresis
        self._last_dominant = 'normal'
        self.hmm_model = None
    ...
    def continuous_regime_prob(self, vix):
        ...
        # Hysteresis: only change dominant regime if new probability exceeds
        # old by the hysteresis band (prevents rapid flip-flopping)
        labels = ['calm', 'normal', 'turbulent']
        new_dominant = labels[np.argmax(raw)]
        prev_idx = labels.index(self._last_dominant) if self._last_dominant in labels else 1
        if new_dominant != self._last_dominant:
            # Only flip if the new regime's probability exceeds old by hysteresis margin
            if raw[np.argmax(raw)] - raw[prev_idx] < self.hysteresis * 0.1:
                new_dominant = self._last_dominant  # stay in current regime
    ...
    def price(self, S, K, T, r, q, sigma, option_type='PE', n_exercise_dates=None):
        ...
        for t_idx in range(len(exercise_dates) - 2, -1, -1):
            ...
            try:
                ...
                # Monotonicity enforcement: continuation value should be
                # non-decreasing in S for calls, non-increasing for puts
                sort_idx = np.argsort(S_itm)
                cont_sorted = continuation[sort_idx]
                if is_call:
                    # Force non-decreasing via isotonic projection
                    for mi in range(1, len(cont_sorted)):
                        if cont_sorted[mi] < cont_sorted[mi - 1]:
                            cont_sorted[mi] = cont_sorted[mi - 1]
                else:
                    # Force non-increasing
                    for mi in range(1, len(cont_sorted)):
                        if cont_sorted[mi] > cont_sorted[mi - 1]:
                            cont_sorted[mi] = cont_sorted[mi - 1]
                # Map back to original order
                inv_idx = np.argsort(sort_idx)
                continuation = cont_sorted[inv_idx]
...
class KellyCriterion:
    ...
    def optimal_fraction(win_prob, avg_win, avg_loss, kelly_fraction=0.5,
        ...
        # Drawdown-state cap: reduce Kelly by 50% if recent PnL is negative
        if payoff_distribution is not None and len(payoff_distribution) >= 5:
            recent_pnl = np.sum(payoff_distribution[-5:])
            if recent_pnl < 0:
                adj_kelly *= 0.5  # halve position during drawdown
...
class BayesianPosteriorConfidence:
    ...
    def __init__(self):
        ...
        # Reliability calibration: track predicted confidence vs realized hit-rate
        self._calibration_bins = {}  # {confidence_decile: (n_correct, n_total)}
        self._brier_score_sum = 0.0
        self._brier_count = 0
    ...
    def update_calibration(self, predicted_confidence: float, was_correct: bool):
        """Track reliability: predicted confidence vs realized hit-rate."""
        # Brier score component
        prob = min(max(predicted_confidence / 100.0, 0.0), 1.0)
        self._brier_score_sum += (prob - float(was_correct)) ** 2
        self._brier_count += 1
        # Bin by confidence decile
        decile = int(predicted_confidence // 10) * 10
        if decile not in self._calibration_bins:
            self._calibration_bins[decile] = [0, 0]
        self._calibration_bins[decile][1] += 1
        if was_correct:
            self._calibration_bins[decile][0] += 1

    def reliability_report(self) -> dict:
        """Return calibration reliability metrics."""
        brier = self._brier_score_sum / max(self._brier_count, 1)
        cal = {}
        for decile, (c, t) in sorted(self._calibration_bins.items()):
            if t >= 5:
                cal[f'{decile}-{decile+10}%'] = {
                    'predicted_avg': decile + 5,
                    'realized_hit_rate': round(c / t * 100, 1),
                    'n': t,
                }
        return {'brier_score': round(brier, 4), 'calibration': cal}
...
class CrossAssetMonitor:
    ...
    def vix_spillover_signal(cboe_vix, india_vix, cboe_vix_prev_close=None,
        ...
        Parameters
        ----------
        data_age_seconds : float - how old the CBOE VIX data is (stale penalty)
        """
        if cboe_vix is None or india_vix is None:
            return {'signal': 'NEUTRAL', 'strength': 0.0, 'stale_data': True}

        # Stale-data penalty: discount signal if data is old
        stale_penalty = 1.0
        if data_age_seconds > 3600:  # older than 1 hour
            stale_penalty = max(0.2, 1.0 - (data_age_seconds - 3600) / 36000)
        ...
        return {
            'signal': signal,
            'strength': float(strength * stale_penalty),
            'expected_india_vix': float(expected_india_vix),
            'gap': float(gap),
            'overnight_vix_change_pct': float(overnight_spike * 100),
            'stale_data': data_age_seconds > 3600,
            'stale_penalty': float(stale_penalty),
        }

    @staticmethod
    def shrinkage_correlation(series_x, series_y, min_obs=30, target_corr=0.0):
        """
        Estimate correlation with simple shrinkage to reduce noise in short windows.
        """
        if series_x is None or series_y is None:
            return {'sample_corr': 0.0, 'shrunk_corr': 0.0, 'n_obs': 0, 'shrinkage': 1.0}

        x = np.asarray(series_x, dtype=float)
        y = np.asarray(series_y, dtype=float)
        n = min(len(x), len(y))
        if n < 3:
            return {'sample_corr': 0.0, 'shrunk_corr': 0.0, 'n_obs': int(n), 'shrinkage': 1.0}

        x = x[-n:]
        y = y[-n:]
        sample_corr = float(np.corrcoef(x, y)[0, 1]) if np.std(x) > 1e-10 and np.std(y) > 1e-10 else 0.0

        # Small n -> stronger shrinkage toward target correlation.
        shrinkage = float(np.clip((2.0 * min_obs - n) / max(2.0 * min_obs, 1.0), 0.0, 1.0))
        shrunk_corr = (1.0 - shrinkage) * sample_corr + shrinkage * float(target_corr)

        return {
            'sample_corr': float(sample_corr),
            'shrunk_corr': float(shrunk_corr),
            'n_obs': int(n),
            'shrinkage': float(shrinkage),
        }

    @staticmethod
    def correlation_signal(series_x, series_y, data_age_seconds: float = 0.0,
                           min_obs: int = 30):
        """
        Cross-asset correlation signal with shrinkage and stale-data penalty.
        """
        corr = CrossAssetMonitor.shrinkage_correlation(
            series_x, series_y, min_obs=min_obs, target_corr=0.0
        )
        stale_penalty = 1.0
        if data_age_seconds > 1800:
            stale_penalty = max(0.2, 1.0 - (data_age_seconds - 1800) / 21600)

        adjusted_corr = float(corr['shrunk_corr'] * stale_penalty)
        if adjusted_corr > 0.7:
            signal = 'STRONG_POSITIVE_LINK'
        elif adjusted_corr < -0.5:
            signal = 'STRONG_NEGATIVE_LINK'
        else:
            signal = 'WEAK_OR_NEUTRAL_LINK'

        return {
            **corr,
            'signal': signal,
            'adjusted_corr': adjusted_corr,
            'stale_data': bool(data_age_seconds > 1800),
...
class GEXCalculator:
    ...
    def compute_gex(spot, strikes, call_oi, put_oi, call_gamma, put_gamma,
        ...
        for K in strikes:
            ...
            # Stale OI detection: flag if OI is suspiciously round/unchanged
            _oi_total = c_oi + p_oi
            _is_stale = (_oi_total > 0 and _oi_total % 100 == 0 and c_gamma == 0 and p_gamma == 0)

            # Net: dealer gamma from calls is positive, from puts is negative
            stale_penalty = 0.5 if _is_stale else 1.0
            gex_at_k = (c_oi * c_gamma - p_oi * p_gamma) * spot * 0.01 * lot_size * stale_penalty
...
class AdaptiveMeshPDE:
    ...
    def __init__(self, N_base=200, Nt=200, targeted_only=True):
        self.N_base = N_base
        self.Nt = Nt
        self.targeted_only = targeted_only  # restrict to specific contracts
    ...
    def price(self, S, K, T, r, q, sigma, option_type='PE', is_american=False,
              contract_targeted=True, allow_full_chain=False):
        """
        ...
        (price, greeks_dict)
        """
        if self.targeted_only and (not contract_targeted) and (not allow_full_chain):
            raise ValueError(
                "AdaptiveMeshPDE.targeted_only=True blocks non-targeted/full-chain usage. "
                "Set contract_targeted=True or allow_full_chain=True."
            )
    ...
    def richardson_extrapolation(self, S, K, T, r, q, sigma, option_type='PE',
                                 is_american=False, contract_targeted=True,
        ...
        self.Nt = 200
        p_fine, _ = self.price(
            S, K, T, r, q, sigma, option_type, is_american,
            contract_targeted=contract_targeted, allow_full_chain=allow_full_chain
        )

        # Coarse grid
        self.N_base = 100
        self.Nt = 100
        p_coarse, _ = self.price(
            S, K, T, r, q, sigma, option_type, is_american,
            contract_targeted=contract_targeted, allow_full_chain=allow_full_chain
        )
...
class MacroFeatureEngine:
    ...
    # Known Indian macro event dates (month, day) for release calendar alignment
    _RELEASE_CALENDAR = {
        'rbi_policy': [(2, 8), (4, 9), (6, 8), (8, 10), (10, 6), (12, 8)],
        'budget': [(2, 1)],
        'quarterly_expiry': [(3, 31), (6, 30), (9, 30), (12, 31)],
    }

    @staticmethod
    def release_calendar_align(features: dict, current_date=None) -> dict:
        """
        Align features to release calendar timestamps.

        If within 3 days of a known macro event, flags the features
        to prevent look-ahead bias from calendar drift.
        """
        import datetime
        ...
        near_event = False
        event_name = None
        for ename, dates in MacroFeatureEngine._RELEASE_CALENDAR.items():
            for m, d in dates:
                try:
                    event = datetime.date(current_date.year, m, d)
                    delta = abs((current_date - event).days)
                    if delta <= 3:
                        near_event = True
                        event_name = ename
                        break
        ...
        features['near_macro_event'] = near_event
        features['macro_event_name'] = event_name or 'none'
        return features
...
class QuantEngine:
    ...
    def __init__(self, deterministic_mode=False, seed=42):
        ...
        # Frontier: Hawkes self-exciting jump estimator (Phase 1)
        try:
            from hawkes_jump import HawkesJumpEstimator
            self.hawkes_estimator = HawkesJumpEstimator()
        except ImportError:
            self.hawkes_estimator = None
    ...
    def fit_jump_params(self, returns, vix_series=None):
        """Estimate jump parameters via EM (default) or Hawkes (when flag ON)."""
        # Check if Hawkes flag is enabled
        try:
            from omega_features import get_features
            use_hawkes = getattr(get_features(), 'USE_HAWKES_JUMPS', False)
        except Exception:
            use_hawkes = False

        if use_hawkes and self.hawkes_estimator is not None:
            # Use Hawkes self-exciting jump estimator
            self._jump_params = self.hawkes_estimator.fit(returns)
            if vix_series is not None and len(vix_series) > 0:
                vix_current = float(vix_series[-1]) if hasattr(vix_series, '__len__') else float(vix_series)
                self._jump_params['current_intensity'] = self.hawkes_estimator.current_intensity(vix_current=vix_current)
    ...
    def fit_hawkes_jumps(self, returns, vix_series=None):
        """Explicitly fit Hawkes jump model (regardless of feature flag)."""
        if self.hawkes_estimator is None:
            return self.fit_jump_params(returns, vix_series)
        result = self.hawkes_estimator.fit(returns)
        # Apply VIX modulation if available
        if vix_series is not None and len(vix_series) > 0:
            vix_current = float(vix_series[-1]) if hasattr(vix_series, '__len__') else float(vix_series)
            result['current_intensity'] = self.hawkes_estimator.current_intensity(vix_current=vix_current)
        return result
    ...
    def enhanced_price(self, S, K, T, r, q, sigma, option_type='CE',
                       ...
                       bid_ask_spread=None, volume=None, median_volume=None,
                       capital=None, use_adaptive_pde=False, is_american=False,
                       adaptive_pde_contract_targeted=True,
                       allow_full_chain_adaptive_pde=False):
        """
        ...
        # 8. Calendar effects (release-calendar aligned to reduce timing mismatch)
        calendar_features = self.macro.calendar_effects()
        result['calendar'] = self.macro.release_calendar_align(calendar_features)
        ...
        # 12. Optional adaptive PDE refinement (targeted contracts only by default)
        if use_adaptive_pde:
            try:
                pde_price, pde_greeks = self.adaptive_pde.price(
                    S, K, T, r, q, sigma, option_type=option_type,
                    is_american=bool(is_american),
                    contract_targeted=bool(adaptive_pde_contract_targeted),
                    allow_full_chain=bool(allow_full_chain_adaptive_pde),
                )
                result['adaptive_pde'] = {
                    'enabled': True,
                    'price': float(pde_price),
                    'greeks': pde_greeks,
                    'targeted_only': bool(self.adaptive_pde.targeted_only),
                }
            except Exception as e:
                result['adaptive_pde'] = {
                    'enabled': True,
...
class InformationGeometryDetector:
    ...
    def __init__(self, n_bins=30):
        self.n_bins = n_bins
        self.risk_flag_only = True  # NOT a standalone signal trigger
...
class TransferEntropyRegimePredictor:
    ...
    def compute_transfer_entropy(source, target, lag=1, n_bins=10):
        ...
        # Minimum sample size + stationarity check
        if n < max(lag + 30, 50):
            return 0.0
        # Reject if series is near-constant (non-stationary degenerate)
        if np.std(source) < 1e-8 or np.std(target) < 1e-8:
            return 0.0
...
class MarketMakerInventory:
    ...
    def estimate(spot, strikes, call_oi, put_oi, call_delta=None, put_delta=None,
        ...
        if not strikes or not call_oi or not put_oi:
            return {'mm_delta': 0.0, 'mm_gamma_regime': 'UNKNOWN',
                    'iv_bias_direction': 'NEUTRAL', 'proxy_quality': 0.0}
        ...
        # Proxy-quality scoring and weighting:
        # 0 => mostly approximated Greeks, 1 => mostly market-provided Greeks.
        delta_cov = float(delta_cov / n_strikes)
        gamma_cov = float(gamma_cov / n_strikes)
        proxy_quality = float(np.clip(0.20 + 0.40 * delta_cov + 0.40 * gamma_cov, 0.0, 1.0))
        w = float(np.clip(proxy_quality_weight, 0.0, 1.0))
        quality_scale = (1.0 - w) + w * proxy_quality
        total_mm_delta *= quality_scale
        total_mm_gamma *= quality_scale
        ...
        return {
            ...
            'proxy_quality_scale': float(quality_scale),
        }
...
class OptimalEntryTiming:
    ...
    def compute(mispricing_pct, T, tc_per_unit, daily_decay_rate=0.01,
                market_price=None, fair_value=None, bid_ask_spread=0.0):
        """
        Parameters
        ----------
        ...
        bid_ask_spread  : float - Bid-ask spread in price units
        ...
        try:
            ...
            # Include spread/impact cost in the threshold directly
            spread_cost = float(bid_ask_spread) if isinstance(bid_ask_spread, (int, float)) else 0.0
            effective_tc = tc_per_unit + spread_cost

            # Threshold declines as expiry approaches (option to wait has less value)
            threshold_edge = effective_tc * (1.0 + beta * np.sqrt(days_left))

            # Convert threshold to percentage of market price
            if market_price and market_price > 0:
                threshold_pct = threshold_edge / market_price * 100
            elif fair_value and fair_value > 0:
                threshold_pct = threshold_edge / fair_value * 100
            else:
                threshold_pct = 3.0  # conservative fallback
            ...
            if abs_mispricing > threshold_pct:
                ...
                ref_price = market_price if (market_price and market_price > 0) else 1.0
                if beta > 0 and effective_tc > 0:
                    # Solve: tc × (1 + β√d) / mp * 100 = mispricing_pct
                    ratio = abs_mispricing * ref_price / (100 * effective_tc) - 1
                    if ratio > 0:
                        target_days = (ratio / beta) ** 2
                        optimal_wait = max(0, min(days_left - target_days, days_left))
                    else:
                        optimal_wait = days_left  # mispricing too small, wait until expiry
                else:
                    optimal_wait = 0
...
class MicrostructureAlphaEngine:
    ...
    @staticmethod
    def quote_age_filter(timestamps, max_age_seconds=300):
        """
        Filter out stale quotes older than max_age_seconds.

        Returns mask of valid (fresh) quotes.
        """
        import time
        now = time.time()
    ...
    @staticmethod
    def crossed_market_filter(bid_prices, ask_prices):
        """
        Detect crossed markets (bid > ask) which indicate stale/invalid quotes.

        Returns mask of valid (not crossed) strikes.
        """
        bids = np.asarray(bid_prices, dtype=float)
        asks = np.asarray(ask_prices, dtype=float)
        return asks >= bids
...
class VarianceSurfaceArbitrage:
    ...
    def check_joint_arbitrage(self, iv_surface_dict, returns_history):
        """
        Joint static + calendar arbitrage check.

        Static arb: butterfly spread values must be >= 0
        Calendar arb: total implied variance must be non-decreasing in T

        Parameters
        ----------
        iv_surface_dict : dict {(strike, tenor_days): iv}
        returns_history : np.ndarray

        Returns
        -------
        dict with static_violations, calendar_violations, is_arbitrage_free
        """
        static_violations = []
        calendar_violations = []
        ...
        # Group by tenor
        tenors = {}
        for (K, T), iv in iv_surface_dict.items():
            if T not in tenors:
                tenors[T] = {}
            tenors[T][K] = iv

        # Static arb check: butterfly spread per tenor
        for T, strike_iv in sorted(tenors.items()):
            strikes = sorted(strike_iv.keys())
            for i in range(1, len(strikes) - 1):
                K_lo, K_mid, K_hi = strikes[i-1], strikes[i], strikes[i+1]
                iv_lo, iv_mid, iv_hi = strike_iv[K_lo], strike_iv[K_mid], strike_iv[K_hi]
                # Total variance must be convex in strike
                w_lo = iv_lo**2 * T / 365
                w_mid = iv_mid**2 * T / 365
                w_hi = iv_hi**2 * T / 365
                butterfly = w_lo - 2 * w_mid + w_hi
                if butterfly < -1e-6:
                    static_violations.append({
                        'tenor': T, 'strikes': (K_lo, K_mid, K_hi),
                        'butterfly': float(butterfly),
                    })

        # Calendar arb check: total variance non-decreasing in tenor
        sorted_tenors = sorted(tenors.keys())
        for i in range(1, len(sorted_tenors)):
            T_prev, T_curr = sorted_tenors[i-1], sorted_tenors[i]
            common_strikes = set(tenors[T_prev].keys()) & set(tenors[T_curr].keys())
            for K in common_strikes:
                w_prev = tenors[T_prev][K]**2 * T_prev / 365
                w_curr = tenors[T_curr][K]**2 * T_curr / 365
                if w_curr < w_prev - 1e-6:
                    calendar_violations.append({
                        'strike': K, 'tenors': (T_prev, T_curr),
                        'total_var_prev': float(w_prev),
                        'total_var_curr': float(w_curr),
                    })

        return {
            'static_violations': static_violations,
            'calendar_violations': calendar_violations,
            'n_static': len(static_violations),
            'n_calendar': len(calendar_violations),
            'is_arbitrage_free': len(static_violations) == 0 and len(calendar_violations) == 0,
        }
...
class OptimalExecution:
    ...
    def optimal_trajectory(total_lots, T_minutes, eta, gamma, risk_aversion=1e-5,
                           n_slices=None, liquidity_regime=None):
        """
        ...
        if isinstance(liquidity_regime, dict):
            participation_multiplier = float(liquidity_regime.get('participation_multiplier', 1.0))
            liquidity_label = str(liquidity_regime.get('label', 'custom'))
        elif isinstance(liquidity_regime, str):
            liquidity_label = liquidity_regime.strip().lower()
            participation_multiplier = {
                'illiquid': 0.7,
                'stressed': 0.65,
                'normal': 1.0,
                'liquid': 1.2,
            }.get(liquidity_label, 1.0)
        participation_multiplier = float(np.clip(participation_multiplier, 0.4, 1.6))

        if n_slices is None:
            base_slices = max(3, min(int(T / 2), 20))  # 1 slice per 2 min
            # Lower participation => spread order over more slices.
            n_slices = int(np.clip(round(base_slices / max(participation_multiplier, 1e-6)), 3, 40))
        ...
        return {
            ...
            'n_slices': n_slices,
            'total_lots': int(X),
            'liquidity_regime': liquidity_label,
            'participation_multiplier': round(float(participation_multiplier), 3),
...
class LevyProcessPricer:
    ...
    def __init__(self, N=256, regularization_lambda=0.01):
        self.N = N
        self.reg_lambda = regularization_lambda  # L2 penalty on params
    ...
    def calibrate_from_market(self, S, strikes, market_prices, T, r, q,
        ...
        def objective(params):
            ...
            sse = float(np.sum((model_prices - market)**2))
            # L2 regularisation around conservative priors to stabilise
            # calibration on short/noisy strike windows.
        ...
        reg_term = float(self.reg_lambda) * (
            (sig - 0.15) ** 2 +
            (th + 0.10) ** 2 +
            (nu - 0.20) ** 2
        ) * len(strikes)
        ...
        return {
            ...
            'nu': round(float(nu), 4),
            'rmse': round(rmse, 4),
            'regularization_penalty': round(reg_term, 8),
...
class ContagionGraph:
    ...
    def build_graph(series_dict, max_lag=5, threshold=0.05):
        ...
        """
        threshold = float(np.clip(threshold, 1e-4, 0.5))
        ...
        for i in range(n):
            for j in range(n):
                ...
                if result['significant'] and float(result.get('p_value', 1.0)) <= threshold:
                    # Use -log10(p) weight to avoid extremely large edge weights.
                    edge_w = max(-np.log10(max(float(result['p_value']), 1e-12)), 0.0)
                    if edge_w < 1e-6:
                        continue
                    adjacency[i, j] = edge_w
                    edges.append({
                        'from': names[i], 'to': names[j],
                        'f_stat': result['f_stat'],
                        'p_value': result['p_value'],
                        'lag': result['optimal_lag'],
                        'edge_weight': round(float(edge_w), 4),
                    })
...
class NeuralSDECalibrator:
    ...
    def calibrate(self, S, strikes, market_prices, T, r, q, V0, kappa, theta,
        ...
        # OOS-only scoring: use last 20% of strikes as hold-out validation
        n_oos = max(1, len(strikes) // 5)
        oos_rmse = float(np.sqrt(np.mean((final_prices[-n_oos:] - market[-n_oos:])**2)))

        return {
            'rmse_before': round(rmse_before, 4),
            'rmse_after': round(rmse_after, 4),
            'rmse_oos': round(oos_rmse, 4),
            'improvement_pct': round((1 - rmse_after / max(rmse_before, 0.01)) * 100, 1),
            'converged': rmse_after < rmse_before and oos_rmse < rmse_before,
            'generations': gen + 1,
        }
...
class RegimeCopula:
    ...
    def analyze(series_x, series_y, regime_prob_crisis=0.5):
        ...
        # Rolling stability validation: unstable dependence should reduce
        # confidence in any downstream tail-based signal.
        stability = RegimeCopula.validate_tail_stability(x, y)
        stable = bool(stability.get('is_stable', True))
        confidence_multiplier = 1.0 if stable else 0.7
        ...
        if not stable:
            blended_rho *= confidence_multiplier
            signal = f"{signal}_UNSTABLE"
        ...
        return {
            ...
            'tail_stability': stability,
            'signal': signal,
        }

    @staticmethod
    def validate_tail_stability(series_x, series_y, window_size=60, step=20):
        """
        Validate tail dependence stability using rolling windows.

        Checks that Kendall's tau and tail dependence don't fluctuate
        wildly across time, which would indicate unstable copula fit.

        Returns
        -------
        dict with is_stable, tau_std, lambda_std, n_windows
        """
        x = np.asarray(series_x, dtype=float)
        y = np.asarray(series_y, dtype=float)
        n = min(len(x), len(y))
        if n < window_size + step:
            return {'is_stable': True, 'n_windows': 0,
                    'reason': 'insufficient data for rolling validation'}

        taus = []
        lambdas = []
        for start in range(0, n - window_size, step):
            end = start + window_size
            tau_w = RegimeCopula.fit_kendall_tau(x[start:end], y[start:end])
            taus.append(tau_w)
            # Quick Clayton tail dependence
            tau_c = max(tau_w, 0.01)
            theta_c = max(2 * tau_c / (1 - tau_c + 1e-6), 0.01)
            lambdas.append(2**(-1/theta_c))

        tau_std = float(np.std(taus))
        lambda_std = float(np.std(lambdas))

        # Stable if std of rolling tau < 0.15 and lambda < 0.2
        is_stable = tau_std < 0.15 and lambda_std < 0.2

        return {
            'is_stable': is_stable,
            'tau_mean': round(float(np.mean(taus)), 4),
            'tau_std': round(tau_std, 4),
            'lambda_mean': round(float(np.mean(lambdas)), 4),
            'lambda_std': round(lambda_std, 4),
            'n_windows': len(taus),
        }
