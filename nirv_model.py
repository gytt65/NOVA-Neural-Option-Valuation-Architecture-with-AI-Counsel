...
# Novel Nifty India Architecture Flags
try:
    from structural_frictions import StructuralFrictions
    from behavioral_agents import BehavioralAgents
    from martingale_optimal_transport import MartingaleOptimalTransport
    USE_NOVEL_ARCHITECTURE = True
except ImportError:
    USE_NOVEL_ARCHITECTURE = False
...
class IndiaFeatureEngine:
    ...
    def compute_features(self, india_vix, fii_net_flow, dii_net_flow,
        ...
        **kwargs       : dict  - Extra features (e.g. 'india_vix_synth')
        ...
        vix_z = np.clip((india_vix - self.vix_mean) / self.vix_std, -4.0, 4.0)
...
class RegimeDetector:
    ...
    # Per-route accuracy tracking for Tier0/1/2
    _route_accuracy = {
        'hmm_trained': {'correct': 0, 'total': 0, 'uses': 0},     # Tier0: full HMM
        'hmm_manual': {'correct': 0, 'total': 0, 'uses': 0},      # Tier1: manual forward
        'fallback': {'correct': 0, 'total': 0, 'uses': 0},        # Tier2: conservative fallback
    }
    ...
    def _register_route_use(self, route_name: str):
        route = str(route_name)
        stats = self._route_accuracy.setdefault(route, {'correct': 0, 'total': 0, 'uses': 0})
        stats['uses'] = int(stats.get('uses', 0)) + 1
        self._last_route = route

    def update_route_outcome(self, was_correct: bool, route_name: str = None):
        """
        Record realized correctness for a regime-detection route.
        Hook this after trade outcomes are known.
        """
        route = str(route_name or self._last_route or '')
        if not route:
            return
        stats = self._route_accuracy.setdefault(route, {'correct': 0, 'total': 0, 'uses': 0})
    ...
    def get_route_stats(self):
        """
        Return route usage and realized-accuracy diagnostics.
        """
        out = {}
        for route, stats in self._route_accuracy.items():
    ...
    def route_quality_weight(self, route_name: str = None, min_samples: int = 20) -> float:
        """
        Convert realized route accuracy into a conservative confidence weight.
        """
        route = str(route_name or self._last_route or '')
        stats = self._route_accuracy.get(route, {})
        total = int(stats.get('total', 0))
        correct = int(stats.get('correct', 0))
        if total < max(int(min_samples), 1):
            return 1.0
        acc = float(correct / max(total, 1))
        # 50% accuracy -> 0.80x, 100% -> 1.10x
        return float(np.clip(0.5 + 0.6 * acc, 0.70, 1.10))
    ...
    def detect_regime(self, returns_30d, india_vix, fii_net_flow,
        ...
        spot_price     : float - Current spot price (for gamma flip check)
        zero_gamma_strike : float - GEX zero-gamma strike (for gamma flip check)

        Returns
        -------
        (regime_name: str, probabilities: dict)
        """
        _route_hint = 'hmm_manual'
        ...
        if self._hmm_model is not None and HMM_AVAILABLE:
            try:
                ...
                self._register_route_use('hmm_trained')
                return regime, probs
            except Exception as e:
                warnings.warn(f"NIRV: hmmlearn regime detection failed ({e}), falling back to manual HMM")
                pass  # Fall through to manual HMM

        # ---- PATH 2: Manual single-step HMM forward (fallback) ----
        # Exponential decay weights: recent returns matter more for regime switching
        _n_ret = len(returns_30d)
        _exp_weights = np.exp(-np.arange(_n_ret)[::-1] / 10.0)  # half-life ~10 days
        _exp_weights /= np.sum(_exp_weights)  # normalize

        log_likelihoods = {}
        for name, params in self.REGIME_PARAMS.items():
            ll = np.sum(_exp_weights * norm.logpdf(returns_30d, loc=params['mu'], scale=params['sigma']))
            if 'High Vol' in name:
        ...
        # ── GAMMA FLIP OVERRIDE ──────────────────────────────────────────
        # When Spot < Zero Gamma Strike, market makers are SHORT gamma:
        #   - They sell dips and buy rips (amplifying moves)
        #   - Volatility is AMPLIFIED, not dampened
        #   - The market mathematically changes regime across this line
        # Force Bear/High-Vol regime when this condition is true.
        if (spot_price is not None and zero_gamma_strike is not None
                and spot_price > 0 and zero_gamma_strike > 0):
            if spot_price < zero_gamma_strike:
                # Dealers are SHORT gamma — force Bear/High-Vol
                if regime != 'Bear-High Vol':
                    warnings.warn(
                        f"NIRV GAMMA FLIP: Spot ({spot_price:.0f}) < Zero Gamma "
                        f"({zero_gamma_strike:.0f}). Overriding regime from "
                        f"'{regime}' → 'Bear-High Vol'"
                    )
                probs['Bear-High Vol'] = max(probs.get('Bear-High Vol', 0.0) * 3.0, 0.6)
                # Re-normalize
        ...
        self._register_route_use(_route_hint)
        return regime, probs
...
class VolatilitySurface:
    ...
    **Critical design choice**: The ATM implied volatility is anchored
    ...
    def _get_parametric_iv(self, spot, strike, T, regime, india_features, hurst_exponent=None):
        ...
        # ── ATM Implied Volatility — smart anchor ───────────────────
        # For short-dated weeklies (DTE ≤ 7), use live chain ATM IV if
        # available. VIX is a 30-day measure and systematically
        # underprices near-term gamma by 2-4 vol points.
        india_vix = india_features.get('india_vix', 14.0)
        chain_atm_iv = india_features.get('chain_atm_iv')  # from live chain
        dte = max(int(T * 365), 1)

        if chain_atm_iv is not None and chain_atm_iv > 0.01 and dte <= 7:
            atm_iv = float(chain_atm_iv)
        else:
    ...
    def get_implied_vol(self, spot, strike, T, regime, india_features,
        ...
        if getattr(get_features(), "USE_ESSVI_SURFACE", False) and ESSVISurface:
            try:
                ...
                warnings.warn(f"NIRV: eSSVI surface lookup failed for regime={regime} ({e}), using parametric IV")
                pass
        ...
        base_iv = self._get_parametric_iv(
            spot, strike, T, regime, india_features, hurst_exponent=hurst_exponent
        )
        
        if USE_NOVEL_ARCHITECTURE:
            try:
                fii_flow = india_features.get('fii_net_flow', 0.0)
                behavioral = BehavioralAgents()
                return behavioral.apply_behavioral_distortions(spot, strike, T, base_iv, fii_net_flow_cr=fii_flow)
            except Exception:
                pass
    ...
    def calibrate_to_market(self, spot, strikes, market_ivs, T, regime, india_features):
        ...
        india_vix = india_features.get('india_vix', 14.0)
        chain_atm_iv = india_features.get('chain_atm_iv')
        dte = max(int(T * 365), 1)

        if chain_atm_iv is not None and chain_atm_iv > 0.01 and dte <= 7:
            atm_iv = float(chain_atm_iv) + self.REGIME_VOL_ADJ.get(regime, 0.0)
        else:
        ...
        try:
            ...
            # Dynamic OTM filter: widen during high-VIX to keep liquid strikes
            _vix_now = float(india_features.get('india_vix', india_features.get('vix', 15.0)))
            _otm_lm_limit = 0.15 + 0.10 * max((_vix_now - 15.0) / 20.0, 0.0)
            _otm_lm_limit = min(_otm_lm_limit, 0.35)  # hard cap
            ...
            if use_liquidity_weighting:
                ...
                for i, (K, mkt_iv) in enumerate(zip(strikes, market_ivs)):
                    ...
                    # Cap vega floor to prevent zero-vega weight explosion for short-dated OTM
                    _vega_floor = float(spot) * 0.001  # 0.1% of spot
                    vega = max(float(spot) * _norm_pdf(d1) * sqrt_t, _vega_floor)
                    # Robust spread floor: at least 1% of vega-implied spread
                    _spread_floor = max(float(spread), 0.01 * float(spot) * sigma_ref * sqrt_t)
                    liq_weights[i] = vega / (_spread_floor * _spread_floor + eps_spread)
                ...
                for i, (K, mkt_iv) in enumerate(zip(strikes, market_ivs)):
                    ...
                    if use_interval_loss and float(K) in interval_bounds:
                        ...
                        if abs(residual) <= huber_delta:
                            ...
                            # Wing preservation: extra penalty for deep OTM/ITM deviations
                            if abs(lk) > 0.05:
                                wing_weight = 2.0 * (abs(lk) / 0.15)  # scale with distance from ATM
                                wing_penalty += wing_weight * residual ** 2
                            continue
                        error += weights[i] * (2 * huber_delta * abs(residual) - huber_delta**2)
                        # Wing penalty for Huber region too
                        if abs(lk) > 0.05:
                            wing_weight = 2.0 * (abs(lk) / 0.15)
                            wing_penalty += wing_weight * (2 * huber_delta * abs(residual) - huber_delta**2)
                        continue
                    error += weights[i] * residual ** 2
                # Add wing penalty (prevents optimizer from flattening wings)
                return error + 0.5 * wing_penalty
            ...
            if result.success:
                ...
                # 1. Weighted RMSE check — soft accept instead of hard reject.
                # Blend 90% old + 10% new on marginal fits to preserve learning signal.
                rmse = np.sqrt(result.fun / np.sum(weights))
                if rmse > 0.08:
                    # Hard reject: extremely bad fit
                    warnings.warn(f"SVI calibration RMSE {rmse:.4f} > 0.08 for {regime}; rolling back")
                    self.svi_params[regime] = old_params
                    return False
                elif rmse > 0.05:
                    # Soft accept: blend old and new to learn gradually
                    warnings.warn(f"SVI calibration RMSE {rmse:.4f} > 0.05 for {regime}; blending 90/10")
                    for _k in candidate:
                        candidate[_k] = 0.9 * old_params.get(_k, candidate[_k]) + 0.1 * candidate[_k]
...
class HestonJumpDiffusionPricer:
    ...
    def price(self, spot, strike, T, r, q, sigma, regime_params,
        ...
        # -- Control variate: dual BSM + Heston COS for tighter MC estimates --
        bsm_analytical = self._bsm_price(spot, strike, T, r, q, sigma_eff, option_type)
        mc_gbm_price = disc * np.mean(payoffs_gbm)

        # Primary CV: BSM analytical
        cv_adjustment = bsm_analytical - mc_gbm_price

        # Secondary CV: Heston COS (if available via regime_params)
        # This dramatically reduces variance for OTM/ITM where BSM is poor
        cos_price = None
        try:
            _qe = regime_params.get('_quant_engine')
            if _qe is not None and hasattr(_qe, 'heston_cos'):
                V0_cos = max(sigma_eff ** 2, 1e-8)
                cos_price = _qe.heston_cos.price(
                    spot, strike, T, r, q, V0_cos,
                    regime_params.get('kappa', 2.0),
                    regime_params.get('theta_v', V0_cos),
        ...
        if cos_price is not None and np.isfinite(cos_price) and cos_price > 0:
            # Dual control variate: optimal linear combination
            # price_cv = price_mc + β₁(BSM - mc_gbm) + β₂(COS - mc_gbm)
            # β_opt = Cov(Y, X) / Var(X) for single CV; extend to multivariate
            # Simplified: weighted blend of both CV adjustments
            cv_bsm = bsm_analytical - mc_gbm_price
            cv_cos = cos_price - mc_gbm_price
            # Use both: weight COS more for OTM (where BSM deviates more)
            moneyness = strike / spot
            cos_weight = min(0.6, 0.3 + 0.5 * abs(moneyness - 1.0))
            bsm_weight = 1.0 - cos_weight
            cv_adjustment = bsm_weight * cv_bsm + cos_weight * cv_cos
...
class NIRVModel:
    ...
    def price_option(self, spot, strike, T, r, q, option_type, market_price,
        ...
        regime, regime_probs = self.regime_detector.detect_regime(
            ...
            zero_gamma_strike=kwargs.get("zero_gamma_strike"),
        )
        regime_route = str(getattr(self.regime_detector, "_last_route", "") or "unknown")
        route_stats = self.regime_detector.get_route_stats()
        ...
        # 6a. Novel Nifty Architecture: Structural STT/Margin Frictions
        if USE_NOVEL_ARCHITECTURE:
            try:
                frictions = StructuralFrictions(spot, strike, T, r)
                
                # Apply structural Gap Risk premium
                is_overnight = (datetime.datetime.now().time() > datetime.time(15, 15))
                sigma = frictions.overnight_gap_risk_adjustment(sigma, is_overnight)
                
                # Apply STT distortional exercise boundary on expiry day
                if T < 1.0 / 365.0:
                    intrinsic = max(spot - strike, 0) if option_type.upper() == 'CE' else max(strike - spot, 0)
                    if intrinsic > 0:
                        eff_val = frictions.apply_stt_exercise_distortion(option_type, intrinsic)
                        if eff_val == 0.0:
                            fair_value = 0.0
            except Exception:
                pass
        ...
        # Optional: route-quality weighting (default OFF) to downweight weak
        # regime inference paths until enough realized outcome evidence exists.
        route_quality_applied = False
        route_quality_weight = 1.0
        if bool(kwargs.get("use_route_quality_weighting", False)):
            route_quality_weight = self.regime_detector.route_quality_weight(regime_route)
            confidence = float(np.clip(confidence * route_quality_weight, 0.0, 99.9))
            route_quality_applied = True
        ...
        greeks['quant_extras']['regime_route'] = regime_route
        greeks['quant_extras']['route_stats'] = route_stats
        greeks['quant_extras']['route_quality_weight'] = float(route_quality_weight)
        greeks['quant_extras']['route_quality_applied'] = bool(route_quality_applied)
    ...
    def record_regime_route_outcome(self, was_correct: bool, route_name: str = None):
        """
        Public hook to update realized correctness of regime-detection routes.
        """
        self.regime_detector.update_route_outcome(was_correct=was_correct, route_name=route_name)
